...
class AsyncCheckpoint:
    ...
    def make_snapshot(self, model, optimizer, epoch, use_timer, step_cnt, timer_record_file, use_copy_, snapshot_stream, device):
        # with self.thread_lock:
        #     asyncio.run(self.allreduce_semaphore.acquire())

        checkpoint_thread = threading.Thread(
            target=self._snapshot_thread,
            args=(model, optimizer, epoch, use_copy_, use_timer, step_cnt, timer_record_file, snapshot_stream, device)
        )
        checkpoint_thread.start()
        return checkpoint_thread
        
    def _snapshot_thread(self, model, optimizer, epoch, use_copy_, use_timer, step_cnt, timer_record_file, snapshot_stream, device):
        if use_timer and step_cnt > 10:
            start_time = time.perf_counter()
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            loop.run_until_complete(self._make_snapshot(model, optimizer, epoch, use_copy_, snapshot_stream, device))
        finally:
            loop.close()
        if use_timer and step_cnt > 10:
    ...
    async def _make_snapshot(self, model, optimizer, epoch, use_copy_, snapshot_stream, device):
        ...
        with torch.cuda.stream(snapshot_stream):
            if use_copy_:
                for param_name, model_tensor_gpu in model.state_dict().items():
                    print(param_name, model_tensor_gpu)
                    model_tensor_cpu = torch.empty_like(model_tensor_gpu, device='cpu').pin_memory()
                    model_tensor_cpu.copy_(model_tensor_gpu)
                # torch.cuda.synchronize()
            else:
                cpu_state_dict = {key: value.cpu() for key, value in model.state_dict().items()}

            optimizer_state_dict = optimizer['optimizer_state_dict']
            fp32_groups_flat = optimizer['fp32_groups_flat']
            ...
            for k, v in optimizer_state_dict['state'].items():
                if torch.is_tensor(v):
                    if use_copy_: 
                        ...
                        optimizer_tensor_cpu = torch.empty_like(optimizer_tensor_gpu, device='cpu').pin_memory() 
                        optimizer_tensor_cpu.copy_(optimizer_tensor_gpu) 
                    else:
                        optimizer_tensor_cpu = v.cpu()
            # print("optimizer_state_dict['state']", optimizer_state_dict['state'])
            for t in fp32_groups_flat:
                if torch.is_tensor(t):
                    if use_copy_: 
                        optimizer_tensor_gpu = t
                        optimizer_tensor_cpu = torch.empty_like(optimizer_tensor_gpu, device='cpu').pin_memory() 
                        optimizer_tensor_cpu.copy_(optimizer_tensor_gpu) 
                    else:
                        optimizer_tensor_cpu = t.cpu()
        # torch.cuda.synchronize()
        # self.allreduce_semaphore.release()
...
def save_checkpoint(iteration, model, optimizer, opt_param_scheduler):
    ...
    if args.deepspeed:
        ...
        ckpt_dir = os.path.join(args.save, 'global_step{:d}'.format(iteration))
        os.makedirs(ckpt_dir, exist_ok=True)
        trace_dir = os.path.join(args.save, "trace")
        ...
        async_ckpt = AsyncCheckpoint(ckpt_dir)
        if prof_save:
            with profiler_context as p:
                with record_function("snapshot_and_save"):
                    # print("making snapshot", f'cuda:{mpu.get_data_parallel_rank()}')
                    # async_ckpt.make_snapshot(model[0], optimizer.state_dict(), epoch=1, use_timer=False, step_cnt=1, timer_record_file=None, use_copy_=False, snapshot_stream=torch.cuda.Stream(), device=f'cuda:{mpu.get_data_parallel_rank()}')
        ...
        # print("model[0].module:", model[0].module)
        # print("optimizer:", optimizer)
        # print("optimizer.state_dict():", optimizer.state_dict())
        # print("checkpoint_name:", checkpoint_name)
        # print("ckpt_dir:", ckpt_dir)
        # print("model[0].module.state_dict:", model[0].module.state_dict)
        # print("state_dict:", state_dict)
        # print("original_state_dict:", original_state_dict)
        
        
        # checkpoint_thread = async_ckpt.make_snapshot(model[0].module, optimizer, epoch, use_timer=args.use_timer, step_cnt=step_cnt, timer_record_file=snapshot_timer_record_file, use_copy_=args.use_copy, snapshot_stream=snapshot_stream, device=device)
        
        # async_ckpt.save_checkpoint(model[0], optimizer, ckpt_dir+'/mp_rank_00_model_states.pt')
...
def save_checkpoint_ds(iteration, model, optimizer, opt_param_scheduler):
    """Save a model checkpoint."""
    args = get_args()
