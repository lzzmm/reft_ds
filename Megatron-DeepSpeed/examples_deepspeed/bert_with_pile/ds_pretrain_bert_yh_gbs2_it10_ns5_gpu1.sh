...
# train_iters_in_million=0.002
train_iters_in_million=4
train_iters=10
...
num_gpus=1 #$(($(ds_ssh nvidia-smi --query-gpu=name --format=csv,noheader | wc -l)-2))
num_gpus_pernode=1 #$(nvidia-smi --query-gpu=name --format=csv,noheader | wc -l)
...
batch_size=1 #16 # $(( ${global_batch_size} / ${dp_size} ))
###############################################################################
### Misc configs
log_interval=100
eval_iters=10
...
data_home="/home/datasets/yxwang/wikipedia"
# data_home="/vc_data_blob/users/conglli/the_pile_bert"
if [[ "$host" == *"webxt"* ]]; then
    data_home="/home/datasets/yxwang/wikipedia"
    # data_home="/blob/data/the_pile_bert"
fi
data_path="${data_home}/wikioutput/wikioutput_text_sentence"
# data_path="${data_home}/pile_bert_train_text_sentence"

vocab_path="${data_home}/vocab/bert-large-uncased-vocab.txt"
...
username=$(whoami)
output_home="/home/datasets/yxwang/megatron/bert/checkpoints/bert_345m/31_ds"
