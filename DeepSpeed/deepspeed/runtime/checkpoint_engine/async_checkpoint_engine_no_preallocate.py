...
class AsyncCheckpointEngine(CheckpointEngine):
    ...
    def save(self, state_dict, path: str, use_copy_=True, snapshot_stream=None, parity_stream=torch.cuda.Stream(), ckpt_args_dict={}):
        ...
        # if ckpt_args_dict["data_parallel_rank"] == 0:
        #     info_dir = "/hpc2hdd/home/zli755/xueze/reft_ds/Megatron-DeepSpeed/examples_deepspeed/data_efficiency/gpt/info"
        #     time_stamp = datetime.now().strftime("%m%d-%H%M%S")
        #     info_path = os.path.join(info_dir, f"{time_stamp}_tp_{ckpt_args_dict['tensor_model_parallel_rank']}_pp_{ckpt_args_dict['pipeline_model_parallel_rank']}_state_dict.txt")
    ...
    def make_snapshot(self, state_dict, use_copy_, snapshot_stream, ckpt_args_dict):
    ...
    def make_snapshot_sync(self, state_dict, use_copy_, snapshot_stream, device, ckpt_args_dict):
